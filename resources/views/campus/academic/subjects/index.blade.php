...
@section('title', __('campus.academic.subjects.title'))

@section('content_header')
    <h1>{{ __('campus.academic.subjects.header', ["campus" => $campus->Nombre]) }}</h1>
@stop
    ...
    <div class="card">
        <div id="wrapper" class="card-body">
            <table id="subjects" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        <th>{{ __('campus.academic.subjects.table.headers.code') }}</th>
                        <th>{{ __('campus.academic.subjects.table.headers.abrevation') }}</th>
                        <th>{{ __('campus.academic.subjects.table.headers.name') }}</th>
                        <th>{{ __('campus.academic.subjects.table.headers.status') }}</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr>
    ...
    <script>
        $(function () {
            let table = $("#subjects").DataTable({
                ...
                "responsive": true,
                "autoWidth": false,
                "ajax": "{{ route('campus.academic.subjects.index', $campus) }}",
                "columns": [
                    { "data": "Codigo" },
                    { "data": "Abreviacion" },
                    { "data": "Nombre" },
                ...
                "buttons": [
                    {
                        ...
                        "title": "{{ __('campus.academic.subjects.table.title', ["campus" => $campus->Nombre]) }}",
                    },
                    {
                        "extend": 'copyHtml5',
