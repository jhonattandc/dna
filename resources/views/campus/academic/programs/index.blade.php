...
@section('title', __('campus.academic.programs.title'))

@section('content_header')
    <h1>{{ __('campus.academic.programs.header', ["campus" => $campus->Nombre]) }}</h1>
@stop
    ...
    <div class="card">
        <div id="wrapper" class="card-body">
            <table id="programs" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        <th>{{ __('campus.academic.programs.table.headers.code') }}</th>
                        <th>{{ __('campus.academic.programs.table.headers.name') }}</th>
                        <th>{{ __('campus.academic.programs.table.headers.evaluation') }}</th>
                        <th>{{ __('campus.academic.programs.table.headers.status') }}</th>
                        <th>{{ __('campus.academic.programs.table.headers.preregistration') }}</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr>
...
@section('js')
    ...
    <script>
        $(function () {
            let table = $("#programs").DataTable({
                "processing": true,
                "serverSide": true,
                "responsive": true,
                "autoWidth": false,
                "ajax": "{{ route('campus.academic.programs.index', $campus) }}",
                "columns": [
                    { "data": "Codigo" },
                    { "data": "Nombre" },
                    { "data": "Tipo_evaluacion" },
                    { "data": "Estado" },
                    { "data": "Aplica_preinscripcion" }
                ],
                "dom": "<'row'<'col-sm-12'l>>" +
                ...
                "buttons": [
                    {
                        ...
                        "title": "{{ __('campus.academic.programs.table.title', ["campus" => $campus->Nombre]) }}",
                    },
                    {
                        "extend": 'copyHtml5',
