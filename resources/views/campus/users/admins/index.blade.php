...
@section('title', __('users.admins.title'))

@section('content_header')
    <h1>{{ __('users.admins.header', ["campus" => $campus->Nombre]) }}</h1>
@stop
    ...
    <div class="card">
        <div id="wrapper" class="card-body">
            <table id="admins" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        <th>{{ __('users.admins.table.headers.code') }}</th>
                        <th>{{ __('users.admins.table.headers.first_name') }}</th>
                        <th>{{ __('users.admins.table.headers.last_name') }}</th>
                        <th>{{ __('users.admins.table.headers.ci') }}</th>
                        <th>{{ __('users.admins.table.headers.email') }}</th>
                        <th>{{ __('users.admins.table.headers.phone') }}</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr>
    ...
    <script>
        $(function () {
            let table = $("#admins").DataTable({
                ...
                "responsive": true,
                "autoWidth": false,
                "ajax": "{{ route('campus.users.admins.index', $campus) }}",
                "columns": [
                    { "data": "Codigo" },
                    { "data": "Primer_nombre" },
                ...
                "buttons": [
                    {
                        ...
                        "title": "{{ __('users.admins.table.title', ["campus" => $campus->Nombre]) }}",
                    },
                    {
                        "extend": 'copyHtml5',
