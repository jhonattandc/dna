...
class CampusAdministrativosSync implements ShouldQueue
{
    ...
    public function handle(Q10API $httpClient)
    {
        ...
        if (!$httpClient->check_end($response)) {
            CampusAdministrativosSync::dispatch($this->campus, $this->offset+1);
        }
        $collection = $httpClient->get_collection($response);

        foreach ($collection as $object_json) {
            $administrativo = Administrativo::updateOrCreate(
                [
            ...
            // Associate the usuario indentificacion for the administrativo.
            ...
            // Associate the tipo indentificacion for the user.
            $tipo_id = $this->campus->tipos_id()->where('Codigo', $object_json['Codigo_tipo_identificacion'])->first();
            if ($tipo_id) {
                $administrativo->tipo_id()->associate($tipo_id);
                $administrativo->save();
            }
        }
    }
