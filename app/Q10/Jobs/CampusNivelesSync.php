...
class CampusNivelesSync implements ShouldQueue
{
    ...
    public function handle(Q10API $httpClient)
    {
        ...
        $response = $httpClient->get_page('niveles', [
            'headers' => [
        ...
        if (!$httpClient->check_end($response)) {
            $this->batch()->add(
                new CampusNivelesSync($this->campus, $this->offset+1)
            );
        }
        $collection = $httpClient->get_collection($response);

        foreach ($collection as $object_json) {
            $this->campus->niveles()->updateOrCreate(
